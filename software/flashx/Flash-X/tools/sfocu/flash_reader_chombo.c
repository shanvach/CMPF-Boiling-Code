...
FR_Block *FR_GetBlock_HDF5_Chombo(FR_File *file, int varPos, int block_no) {
	...
	gid = H5Gopen(handle, refine_level_str, H5P_DEFAULT);
	
	/* get offsets */
	dset = H5Dopen(gid, "data:offsets=0", H5P_DEFAULT);
	...
	/* Get Data */
	dset = H5Dopen(gid, "data:datatype=0", H5P_DEFAULT);
	dspace = H5Dget_space(dset);
...
FR_File* FR_open_HDF5_Chombo(hid_t handle, FR_File *out, options_t* opts){
	...
	/*Dimensions*/
	gid= H5Gopen(handle, "Chombo_global", H5P_DEFAULT);
	...
	for(i =0; i < nRefineLevels; i++){
		...
		gid= H5Gopen(handle, level_name, H5P_DEFAULT);

		dset= H5Dopen(gid, "boxes", H5P_DEFAULT);
		dspace = H5Dget_space(dset);
