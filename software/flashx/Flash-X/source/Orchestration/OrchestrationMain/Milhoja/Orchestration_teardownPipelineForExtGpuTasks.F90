...
!!
!! @brief Concrete implementation of Orchestration_teardownPipelineForExtGpuTasks
#include "Milhoja.h"
subroutine Orchestration_teardownPipelineForExtGpuTasks(nThreads, nTilesPerPacket)
    ...
    use milhoja_runtime_mod, ONLY : milhoja_runtime_teardownPipelineForExtGpuTasks
#endif
...
#if ! defined(RUNTIME_MUST_USE_TILEITER) && defined(RUNTIME_SUPPORT_DATAPACKETS)
    CALL milhoja_runtime_teardownPipelineForExtGpuTasks(MH_nThreads, MH_nTilesPerPacket, MH_ierr)
    CALL Orchestration_checkInternalError("Orchestration_teardownPipelineForExtGpuTasks", MH_ierr)
#else
    CALL Driver_abort("Orchestration_teardownPipelineForExtGpuTasks: milhoja_runtime_teardownPipelineForExtGpuTasks disabled")
#endif
