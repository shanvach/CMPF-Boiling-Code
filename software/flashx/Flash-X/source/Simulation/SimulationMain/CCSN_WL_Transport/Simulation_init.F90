...
!  call RuntimeParameters_get('xo16', sim_xo16)
   ...
   if (sim_restart) then
      call IO_getPrevScalar("postBounce", sim_postBounce, ierr)
      if (ierr .ne. 0) call Driver_abort("Failed to read postBounce from checkpoint file")

      if (sim_postBounce) then
         call IO_getPrevScalar("bounceTime", sim_bounceTime, ierr)
         if (ierr .ne. 0) call Driver_abort("Failed to read bounceTime from checkpoint file")
      end if
   end if
