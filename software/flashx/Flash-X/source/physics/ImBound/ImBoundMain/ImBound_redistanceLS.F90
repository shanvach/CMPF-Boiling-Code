!!***if* source/physics/ImBound/ImBoundMain/ImBound_redistanceLS
!! NOTICE
...
!-----------------------------------------------------------------------------------------
   ...
   call Timers_start("ImBound_redistanceLS")
   ...
   if (iteration .eq. 1) then
      solnData(HLN0_VAR, :, :, :) = solnData(LMDA_VAR, :, :, :)
   end if
...
#if NDIM < MDIM
   !--------------------------------------------
   ! Call LMDA re-initialization routine for 2D:
   !--------------------------------------------
   call Stencils_lsRedistance2d(solnData(LMDA_VAR, :, :, :), &
                                solnData(HLN0_VAR, :, :, :), &
                                lsDT, del(DIR_X), del(DIR_Y), &
                                stnLimits(LOW, IAXIS), stnLimits(HIGH, IAXIS), &
                                stnLimits(LOW, JAXIS), stnLimits(HIGH, JAXIS))
#else
   call Stencils_lsRedistance3d(solnData(LMDA_VAR, :, :, :), &
                                solnData(HLN0_VAR, :, :, :), &
   ...
   call Timers_stop("ImBound_redistanceLS")
